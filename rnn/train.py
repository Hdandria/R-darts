...
from utils import batchify, create_exp_dir, get_batch, repackage_hidden, save_checkpoint
...
parser.add_argument("--nhid", type=int, default=850, help="number of hidden units per layer")
parser.add_argument(
    "--nhidlast", type=int, default=850, help="number of hidden units for the last rnn layer"
)
...
parser.add_argument(
    "--dropout", type=float, default=0.75, help="dropout applied to layers (0 = no dropout)"
)
parser.add_argument(
    "--dropouth",
    type=float,
...
if args.continue_train:
    ...
    if isinstance(model.rnns[0], model_module.DARTSCell):
        genotype = model.rnns[0].genotype
else:
    ...
    model = model_module.RNNModel(
        ...
        args.dropouti,
        args.dropoute,
        cell_cls=model_module.DARTSCell,
        genotype=genotype,
    )

parallel_model: nn.Module
if args.cuda:
    if args.single_gpu:
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        while start < args.batch_size:
            ...
            log_prob, new_hidden, rnn_hs, dropped_rnn_hs = parallel_model(
                cur_data, hidden[s_id], return_h=True
            )
            hidden[s_id] = new_hidden
...
logging.info("=" * 89)
logging.info(
    f"| End of training | test loss {test_loss:5.2f} | test ppl {math.exp(test_loss):8.2f}"
)
logging.info("=" * 89)
