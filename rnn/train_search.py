...
import model_search as model_search_module
import numpy as np
import torch
...
parser.add_argument(
    "--nhidlast", type=int, default=300, help="number of hidden units for the last rnn layer"
)
...
parser.add_argument(
    "--arch_lr", type=float, default=3e-3, help="learning rate for the architecture encoding alpha"
)
...
model: model_search_module.RNNModelSearch
...
for p in model.parameters():
    size += p.nelement()
logging.info(f"param size: {size}")
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        while start < args.batch_size:
            ...
            cur_data_valid, cur_targets_valid = (
                ...
                targets_valid[:, start:end].contiguous().view(-1),
            )
